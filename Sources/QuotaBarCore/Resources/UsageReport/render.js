...
(() => {
  const COMMON_COPY = {
    zh: {
      totalLabel:'Token 总用量',costLabel:'估算 API 费用',cacheReadLabel:'缓存读取',cacheWriteLabel:'缓存写入',
      ...
      costNote:'* 为部分费用估算，未定价 tokens 未计入。金额为按当日价格估算的 API 费用，不是实际账单。',
      fullCostNote:'金额为按当日价格估算的 API 费用，不是实际账单。',
      unpricedCostNote:'此区间的用量均未定价，无法提供费用估算。',
      cacheCostNote:'缓存读取、写入费用已计入总额，但不单独列出，这里只展示缓存 token 用量。',
      ...
      zeroDailyTitle:'本期已存用量为零',missingDailyTitle:'本期没有每日记录',noCost:'本期没有非零模型费用',noPricedCost:'本期没有已定价模型费用',
    ...
    en: {
      totalLabel:'Total tokens',costLabel:'Estimated API cost',cacheReadLabel:'Cache read',cacheWriteLabel:'Cache write',
      ...
      costNote:'* Partial estimate. Unpriced tokens are excluded. Amounts are API cost estimates at the prices of each day, not actual bills.',
      fullCostNote:'Amounts are API cost estimates at the prices of each day, not actual bills.',
      unpricedCostNote:'All usage in this period is unpriced. No cost estimate is available.',
      cacheCostNote:'Cache read and write costs are included in the totals but not broken out. Only cache token counts are shown.',
      ...
      modelsSource:'Estimated model costs · Brighter means higher cost · USD',
      ...
      zeroDailyTitle:'Recorded usage is zero',missingDailyTitle:'No daily records in this period',noCost:'No nonzero model costs',noPricedCost:'No priced model costs in this period',
